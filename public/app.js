...
function openSettings() {
  ...
  $("#associateProtocolInput").checked = settings.associateProtocol === true;
  $("#downloadDirectoryInput").value = settings.downloadDirectory || "";
  $("#allowDevToolsInput").checked = settings.allowDevTools === true;
...
async function saveApplicationSettings() {
  const payload = {
    ...(state.settings || DEFAULT_SETTINGS),
    associateProtocol: $("#associateProtocolInput").checked,
    downloadDirectory: $("#downloadDirectoryInput").value.trim(),
    allowDevTools: $("#allowDevToolsInput").checked
  };
  const saved = await api("/api/settings", {
    method: "PUT",
  ...
  state.settings = saved;
  if (window.itemManager?.applyDesktopSettings) {
    await window.itemManager.applyDesktopSettings(saved);
  }
...
async function loadData() {
  ...
  applyViewSettings(state.settings);
  if (window.itemManager?.applyDesktopSettings) {
    await window.itemManager.applyDesktopSettings(state.settings);
  }
  renderAll();
}
...
$("#pickDownloadDirectoryButton").addEventListener("click", async () => {
  try {
    const result = await chooseLocalPath("folder");
    if (result.localPath) $("#downloadDirectoryInput").value = result.localPath;
  } catch (error) {
    await notify(error.message);
  }
});
